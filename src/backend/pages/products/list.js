...
function ProductList(){
    ...
    const delProduct = async (slug) => {
        apiProduct.delProductBySlug(slug).then(res => {
            try{
                alert("Xóa thành công");
                setDelProductItem(slug);
            }catch(e){
                console.log(e);
            }
        });
                    ...
                    products.map((product) => (
                        <tr key={product.id}>
                            ...
                            <td>
                                ...
                                <Link to={`/admin/editProduct/${product.slug}`}> sửa <FaEdit /></Link>
                                <Link to={`/products/${product.slug}`} onClick={() => delProduct(product.slug)}>Xóa<MdDelete /></Link>
                            </td>
                        </tr>
                ))}
            </table>
